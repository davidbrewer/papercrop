  ...
  window.init_papercrop = function() {
    $("div[id$=_cropbox]").each(function() {
      ...
      hideCropper = function() {
        $('.papercrop_container').removeClass('active');
        $('#papercrop_scrim').remove();
      }

      handleCropDone = function(e) {
        e.preventDefault();
        hideCropper();
      }
      ...
      handleCropCancel = function(e) {
        e.preventDefault();
        $("#" + attachment + "_crop_x").val('');
        $("#" + attachment + "_crop_y").val('');
        $("#" + attachment + "_crop_w").val('');
        $("#" + attachment + "_crop_h").val('');
        hideCropper();
      }

      $('button.papercrop_crop_button').on('click', function(e) {
        e.preventDefault();
        $(this).parents('.papercrop_preview_container').
          siblings('.papercrop_container').
          addClass('active');
